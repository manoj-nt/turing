...
// User represents the ORM model for a user, with embedded Address and Tags slice.
type User struct {
...
func SerializeQueryParams(rawQuery string) (*User, error) {
	...
	for key, values := range params {
		...
		} else if strings.Contains(key, "[") { // Handle nested fields like `address[coordinates][lat]`
