...
func handlePost(w http.ResponseWriter, r *http.Request) {
	...
	params := make(map[string][]string) // Modified to store slices of values
	isValid := make(map[string]bool)

	// Validate parameters and store them
	for key, values := range r.Form {
		paramValues := make([]string, len(values))
		for i, value := range values {
			paramValues[i] = value
		}
		params[key] = paramValues                                         // Store all values for the parameter
		isValid[key] = validateParameter(key, strings.Join(values, ", ")) // Validate all values joined as a single string
	}
